...
sub _parse_file{
  ...
  foreach my $tx_id (keys %TxObj){
    ...
    if ($tx->strand eq "+")
    {
      ...
      if (($#$starts > -1) && ($#$cds > -1)){
        # assumes innermost start codon is included in CDS
        $ilen = $$cds[0]->start - $$starts[$#$starts]->start;
        $overlap++ if ($ilen != 0); # has to be a separate error
      }
      if (($#$cds > -1) && ($#$stops > -1)){
      ...
      if (($#$cds > -1) && ($#$starts > -1)){
        # assumes innermost start codon is included in CDS
        $ilen = $$starts[0]->stop - $$cds[$#$cds]->stop;
        $overlap++ if ($ilen != 0); # has to be a separate error
      }
      if (($#$starts > -1) && ($#$utr5 > -1)){
    ...
    if($overlap > 0){
      if($errors[45] < $max_error_count){
        print $warnings
            "Overlapping CDS or UTR features in transcript \"$tx_id\".\n";
      }
      $errors[45]++;
    ...
    if($zero_intron_count > 0){
      if($errors[48] < $max_error_count){
        print $warnings "$zero_intron_count intron(s) shorter than 4 bp found on in transcript \"$tx_id\".\n" ;
      }
      $errors[48]++;
...
sub _update{
  ...
  #$txs = [sort {$a->start <=> $b->start || $a->stop <=> $b->stop} @$txs];
}
